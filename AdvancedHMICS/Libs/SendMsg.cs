using System;
using System.Net;
using System.Net.Mail;
using System.Windows.Forms;

namespace AdvancedHMICS.Libs
{
    internal class SendMsg
    {
        public static int Send(Sender sender, string Setor, string Processo, string Conteudo )
        {
            try
            {
                SmtpClient smtpcliente = new SmtpClient(sender.GetServer(), sender.GetPort()); ;
                smtpcliente.EnableSsl = true;
                smtpcliente.UseDefaultCredentials = false;
                smtpcliente.Credentials = new NetworkCredential(sender.GetEmail(), sender.GetPass());

                MailMessage mensagem = new MailMessage();
                mensagem.From = new MailAddress("lactalis.ijui@outlook.com");
                mensagem.To.Add("lactalis.ijui@outlook.com");
                mensagem.Subject = Setor;
                mensagem.IsBodyHtml = true;
                mensagem.Body = $"<h1>Registro de Acontecimento</h1><p></p><p>{Setor}</p><p>{Processo}</p><p>{Conteudo}</p>";
                smtpcliente.Send(mensagem);
                return 1;

            }
            catch (Exception ex)
            {
                MessageBox.Show("Erro: " + ex.GetBaseException());
                return 0;
            }
            

        }
    }
}
